	...
	class Worker extends XmlRpc {
		...
		*/
		void writeError( int code, String message, XmlWriter writer ) {
			...
			NSMutableDictionary h = new NSMutableDictionary();
			h.setObjectForKey( new Integer( code ), "faultCode" );
			h.setObjectForKey( message, "faultString" );
			writer.startElement( "methodResponse" );
...
class Invoker implements XmlRpcHandler {
	...
	public Object execute( String methodName, NSArray params ) throws Exception {

		// Array mit Classtype bilden, ObjectAry mit Values bilden	
		Class[] argClasses = null;
		Object[] argValues = null;
		if( params != null ) {
			argClasses = new Class[params.count()];
			argValues = new Object[params.count()];
			for( int i = 0; i < params.count(); i++ ) {
				argValues[i] = params.objectAtIndex( i );
